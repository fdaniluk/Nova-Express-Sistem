...
const resumen = manejar(async (req, res) => res.json(await m.resumen(String(req.query.mes || ''))));
...
const excel = manejar(async (req, res) => {
  ...
  ws.columns = [{ header: 'Vendedor', key: 'v', width: 18 }, { header: 'Cliente', key: 'c', width: 32 }, { header: 'Envíos', key: 'n', width: 8 },
    ...
    { header: '% comisión', key: 'p', width: 11 }, { header: 'Comisión USD', key: 'com', width: 14, style: { numFmt: NUM } }];
  cab(ws);
  const grupos = [...r.vendedores, ...(r.sin_asignar.envios ? [r.sin_asignar] : [])];
  for (const g of grupos) {
    const fila = ws.addRow({ v: g.vendedor, c: `TOTAL ${g.vendedor}`, n: g.envios, venta: g.venta, u: g.utilidad, p: g.es_casa ? 'casa' : g.pct, com: g.comision });
    fila.font = { bold: true };
    for (const c of g.clientes) ws.addRow({ v: '', c: c.cliente, n: c.envios, venta: c.venta, u: c.utilidad, p: c.pct, com: c.comision });
  }
  const t = ws.addRow({ v: 'TOTAL', c: '', n: r.total.envios, venta: r.total.venta, u: r.total.utilidad, p: '', com: r.total.comision }); t.font = { bold: true };
  for (const g of grupos) {
