...
propuesto: A → B1/B4/B5 → B2/B3 → C. **A hecho el 10/09** (`c936721`, desplegado). **B1/B4/B5 hechos el 10/09** (Tax ID largo, título de la proforma, numeración automática desde 1300 en Configuración; paquete entregado, cache `?v=20260910b`). **B2/B3 también hechos** (hoja A4 de CampusShip con instrucciones + doblez + etiqueta acostada; térmica solo etiqueta; botón Girar — verificar orientación con la primera guía real). **C (dashboard) hecho el 10/09** (`DASHBOARD-REDISENO.md`; paquete `dashboard.tgz`, cache `?v=20260910c`, tandas `test-dashboard-analitica` 52 y `test-pantalla-dashboard` 29). **La lista de la oficina del 10/09 queda completa.**
